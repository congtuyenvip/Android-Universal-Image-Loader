package com.ctvip.uil.cache.memory.impl;

import android.graphics.Bitmap;

import com.ctvip.uil.cache.memory.BaseMemoryCache;
import com.ctvip.uil.core.pool.BitmapPoolImpl;
...
public class WeakLRULimitedMemCachePool extends BaseMemoryCache {
     ...
     */
    public WeakLRULimitedMemCachePool(int pLimitCacheSize) {
        this.mLimitCacheSize = pLimitCacheSize;
        mCacheSize = new AtomicInteger();
        if (pLimitCacheSize > MAX_NORMAL_CACHE_SIZE) {
            Logger.w("You set too large memory cache size (more than %1$d Mb)", MAX_NORMAL_CACHE_SIZE_IN_MB);
        }
    }
        ...
        if (valueSize < sizeLimit) {
            while (curCacheSize + valueSize > sizeLimit) {
                ...
                if (removedValue != null && removedValue.getValue() != null) {
                    ...
                    BitmapPoolImpl.putBitmap(removedValue.getValue());
                }
            }

...
public class BaseImageDecoder implements ImageDecoder {
	...
	protected ExifInfo defineExifOrientation(String imageUri) {
		...
		try {
			...
			Logger.w("Can't read EXIF tags from file [%s]", imageUri);
		}
	...
	protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
		...
		if (scale > 1 && loggingEnabled) {
			Logger.d(LOG_SUBSAMPLE_IMAGE, imageSize, imageSize.scaleDown(scale), scale, decodingInfo.getImageKey());
		}
	...
	protected Bitmap considerExactScaleAndOrientatiton(Bitmap subsampledBitmap, ImageDecodingInfo decodingInfo,
		...
		if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
			...
			if (Float.compare(scale, 1f) != 0) {
				...
				if (loggingEnabled) {
					Logger.d(LOG_SCALE_IMAGE, srcSize, srcSize.scale(scale), scale, decodingInfo.getImageKey());
				}
			}
		}
		// Flip bitmap if need
		if (flipHorizontal) {
			...
			if (loggingEnabled) Logger.d(LOG_FLIP_IMAGE, decodingInfo.getImageKey());
		}
		// Rotate bitmap if need
		if (rotation != 0) {
			m.postRotate(rotation);

			if (loggingEnabled) Logger.d(LOG_ROTATE_IMAGE, rotation, decodingInfo.getImageKey());
		}

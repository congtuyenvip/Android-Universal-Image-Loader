 ...
 *******************************************************************************/
package com.ctvip.uil.core;

import android.graphics.Bitmap;
import android.os.Handler;
import com.ctvip.uil.core.assist.FailReason;
import com.ctvip.uil.core.assist.FailReason.FailType;
import com.ctvip.uil.core.assist.ImageScaleType;
import com.ctvip.uil.core.assist.ImageSize;
import com.ctvip.uil.core.assist.LoadedFrom;
import com.ctvip.uil.core.assist.ViewScaleType;
import com.ctvip.uil.core.decode.ImageDecoder;
import com.ctvip.uil.core.decode.ImageDecodingInfo;
import com.ctvip.uil.core.download.ImageDownloader;
import com.ctvip.uil.core.download.ImageDownloader.Scheme;
import com.ctvip.uil.core.imageaware.ImageAware;
import com.ctvip.uil.core.listener.ImageLoadingListener;
import com.ctvip.uil.core.listener.ImageLoadingProgressListener;
...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
	...
	public void run() {
		...
		Logger.d(LOG_START_DISPLAY_IMAGE_TASK, memoryCacheKey);
		if (loadFromUriLock.isLocked()) {
			Logger.d(LOG_WAITING_FOR_IMAGE_LOADED, memoryCacheKey);
		}
		...
		try {
			...
			if (bmp == null || bmp.isRecycled()) {
				...
				if (options.shouldPreProcess()) {
					Logger.d(LOG_PREPROCESS_IMAGE, memoryCacheKey);
					bmp = options.getPreProcessor().process(bmp);
					if (bmp == null) {
						Logger.e(ERROR_PRE_PROCESSOR_NULL, memoryCacheKey);
					}
				}

				if (bmp != null && options.isCacheInMemory()) {
					Logger.d(LOG_CACHE_IMAGE_IN_MEMORY, memoryCacheKey);
					configuration.memoryCache.put(memoryCacheKey, bmp);
				}
			} else {
				loadedFrom = LoadedFrom.MEMORY_CACHE;
				Logger.d(LOG_GET_IMAGE_FROM_MEMORY_CACHE_AFTER_WAITING, memoryCacheKey);
			}

			if (bmp != null && options.shouldPostProcess()) {
				Logger.d(LOG_POSTPROCESS_IMAGE, memoryCacheKey);
				bmp = options.getPostProcessor().process(bmp);
				if (bmp == null) {
					Logger.e(ERROR_POST_PROCESSOR_NULL, memoryCacheKey);
				}
			}
			checkTaskNotActual();
			checkTaskInterrupted();
	...
	private boolean waitIfPaused() {
		...
		if (pause.get()) {
			synchronized (engine.getPauseLock()) {
				if (pause.get()) {
					Logger.d(LOG_WAITING_FOR_RESUME, memoryCacheKey);
					try {
						engine.getPauseLock().wait();
					} catch (InterruptedException e) {
						Logger.e(LOG_TASK_INTERRUPTED, memoryCacheKey);
						return true;
					}
					Logger.d(LOG_RESUME_AFTER_PAUSE, memoryCacheKey);
				}
			}
		}
	...
	private boolean delayIfNeed() {
		if (options.shouldDelayBeforeLoading()) {
			Logger.d(LOG_DELAY_BEFORE_LOADING, options.getDelayBeforeLoading(), memoryCacheKey);
			try {
	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
				Logger.d(LOG_LOAD_IMAGE_FROM_DISK_CACHE, memoryCacheKey);
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				Logger.d(LOG_LOAD_IMAGE_FROM_NETWORK, memoryCacheKey);
	...
	private boolean tryCacheImageOnDisk() throws TaskCancelledException {
		Logger.d(LOG_CACHE_IMAGE_ON_DISK, memoryCacheKey);
		...
		try {
			...
			if (loaded) {
				...
				if (width > 0 || height > 0) {
					Logger.d(LOG_RESIZE_CACHED_IMAGE_FILE, memoryCacheKey);
	...
	private boolean downloadImage() throws IOException {
		...
		if (is == null) {
			Logger.e(ERROR_NO_IMAGE_STREAM, memoryCacheKey);
			return false;
		} else {
			try {
	...
	private boolean resizeAndSaveImage(int maxWidth, int maxHeight) throws IOException {
		...
		if (targetFile != null && targetFile.exists()) {
			...
			if (bmp != null && configuration.processorForDiskCache != null) {
				Logger.d(LOG_PROCESS_IMAGE_BEFORE_CACHE_ON_DISK, memoryCacheKey);
				bmp = configuration.processorForDiskCache.process(bmp);
				if (bmp == null) {
					Logger.e(ERROR_PROCESSOR_FOR_DISK_CACHE_NULL, memoryCacheKey);
				}
			}
			if (bmp != null) {
	...
	private boolean isTaskInterrupted() {
		if (Thread.interrupted()) {
			Logger.d(LOG_TASK_INTERRUPTED, memoryCacheKey);
			return true;
		}
		return false;
	}
